...
from jacal_validator import proposals
from jacal_validator.output import human
...
def _policy(metadata=None) -> dict:
    policy = {
...
def test_metadata_is_rejected_without_the_proposal(tmp_path, json_schemas):
    """The premise of the whole exercise: the published schemas do not admit this."""
    result = _validate(_write(tmp_path, _policy(_METADATA)), json_schemas)
    assert not result.valid


def test_metadata_is_accepted_with_the_proposal(tmp_path, json_schemas):
    result = _validate(_write(tmp_path, _policy(_METADATA)), json_schemas, [PROPOSAL])
    assert result.valid, [i.message for i in result.issues]


def test_a_clean_policy_validates_the_same_either_way(tmp_path, json_schemas):
    """Applying a proposal must not relax anything it did not ask to change."""
    path = _write(tmp_path, _policy())
    assert _validate(path, json_schemas).valid
    assert _validate(path, json_schemas, [PROPOSAL]).valid


def test_an_empty_metadata_is_still_rejected_under_the_proposal(tmp_path, json_schemas):
    """MetadataType has no 'declared but empty' state — that is what minProperties in
    the fragment is for."""
    result = _validate(_write(tmp_path, _policy({})), json_schemas, [PROPOSAL])
    assert not result.valid
...
def test_duplicate_attribute_id_is_rejected(tmp_path, json_schemas):
    document = _policy({"Attribute": [
        {"AttributeId": "urn:oasis:names:tc:acal:1.0:provenance:tool",
         "Value": ["acal-convert/1"]},
        {"AttributeId": "urn:oasis:names:tc:acal:1.0:provenance:tool",
         "Value": ["acal-convert/2"]},
    ]})
    result = _validate(_write(tmp_path, document), json_schemas, [PROPOSAL])
    assert not result.valid
    assert any("metadata-attribute-unique" in (i.rule_id or "") for i in result.issues)


def test_same_attribute_id_from_different_issuers_is_allowed(tmp_path, json_schemas):
    ...
    document = _policy({"Attribute": [
        {"AttributeId": "urn:example:author", "Issuer": "one", "Value": ["alice"]},
        {"AttributeId": "urn:example:author", "Issuer": "two", "Value": ["bob"]},
    ]})
    result = _validate(_write(tmp_path, document), json_schemas, [PROPOSAL])
    assert result.valid, [i.message for i in result.issues]
...
def test_the_result_records_which_proposals_were_applied(tmp_path, json_schemas):
    result = _validate(_write(tmp_path, _policy(_METADATA)), json_schemas, [PROPOSAL])
    assert result.proposals == [PROPOSAL]


def test_the_pass_line_itself_says_the_proposal_is_unadopted(tmp_path, json_schemas):
    """On the outcome line, not in a footnote: that is the line quoted out of context."""
    result = _validate(_write(tmp_path, _policy(_METADATA)), json_schemas, [PROPOSAL])
    out = io.StringIO()
    human(result, "policy.json", file=out)
...
def test_an_ordinary_pass_mentions_no_proposal(tmp_path, json_schemas):
    result = _validate(_write(tmp_path, _policy()), json_schemas)
    out = io.StringIO()
    human(result, "policy.json", file=out)
