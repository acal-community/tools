...
#: Fragment filename per serialization. The JACAL validator reads the JSON fragment; the
#: YACAL validator reads the YAML one. They describe the same change in each schema
#: language's own house style rather than one being generated from the other.
STRUCTURE_FRAGMENT = "json.fragment.json"
...
def proposals_dir() -> Path:
    ...
    if env:
        return Path(env)
    # .../<repo>/yacal-validator/src/jacal_validator/proposals.py
