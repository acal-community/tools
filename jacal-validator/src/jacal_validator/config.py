...
Lookup order:
  1. jacal-validator.toml in the current working directory
  2. ~/.config/jacal-validator/config.toml
  3. Built-in defaults
"""
...
def _read_toml() -> dict | None:
    for path in (
        Path.cwd() / "jacal-validator.toml",
        Path.home() / ".config" / "jacal-validator" / "config.toml",
    ):
        if path.exists():
