"""Evaluator for the ACAL v1.0 higher-order constraint catalog.

Reads the machine-readable constraint catalog (acal-core-yaml-v1.0-constraints.yaml)
and evaluates each rule against a parsed JACAL document.

The constraint catalog is shared with YACAL — the semantic rules and path notation
are language-neutral (evaluated over parsed Python dicts regardless of source format).
...
def _provenance(rule: dict) -> str | None:
    ...
    if "YACALSection" in prov:
        parts.append(f"JACAL §{prov['YACALSection']}")
        ...
        for idx, entry in enumerate(combiner_input):
            ...
            if rule_id_val in seen:
                out.append(ValidationIssue(
                    ...
                    path=item_path,
                    rule_id="jacal:rule-id-unique-within-policy",
                    spec_ref=spec,
                ))
            else:
            ...
            for i, sid in enumerate(short_ids):
                ...
                if name in seen:
                    out.append(ValidationIssue(
                        ...
                        path=item_path,
                        rule_id="jacal:shortidset-shortid-name-unique",
                        spec_ref=spec,
                    ))
                else:
                    seen[name] = item_path
        ...
        for idx, var_def in enumerate(policy.get("VariableDefinition", []) or []):
            ...
            if var_id_str in seen_local:
                out.append(ValidationIssue(
                    ...
                    path=item_path,
                    rule_id="jacal:policy-variable-id-scoped-unique",
                    spec_ref=spec,
                ))
            ...
            for var_idx, var_def in enumerate(rule.get("VariableDefinition", []) or []):
                ...
                if var_id_str in seen_local:
                    out.append(ValidationIssue(
                        ...
                        path=item_path,
                        rule_id="jacal:rule-variable-id-scoped-unique",
                        spec_ref=spec,
                    ))
    ...
    for nid in list(nodes):
        if color[nid] == WHITE and dfs(nid):
            ...
            out.append(ValidationIssue(
                ...
                path=path,
                rule_id="jacal:shortidset-reference-acyclic",
                spec_ref=spec,
            ))
    ...
    for nid, (path, node) in nodes.items():
        ...
        if len(refs) != len(set(refs)):
            out.append(ValidationIssue(
                ...
                path=path,
                rule_id="jacal:shortidset-reference-no-repeat",
                spec_ref=spec,
            ))
        if walk(nid, set()):
            out.append(ValidationIssue(
...
def evaluate(
    ...
    - skipped:   rules that emitted a jacal-skip issue (cross-document dependency)
    """
    ...
    for rule in catalog.get("Rule", []) or []:
        ...
        try:
            ...
            issues.append(ValidationIssue(
                ...
                rule_id=f"jacal-eval-error:{rule.get('Id', '?')}",
            ))
        new_issues = issues[before:]
        if any((i.rule_id or "").startswith("jacal-skip:") for i in new_issues):
            skipped += 1
        else:
            evaluated += 1
    ...
    for check in _SUPPLEMENTARY_CHECKS:
        ...
        try:
            ...
            issues.append(ValidationIssue(
                ...
                rule_id=f"jacal-eval-error:{check.__name__}",
            ))
        evaluated += 1
